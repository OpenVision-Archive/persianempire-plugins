...
class showPurePrestigetoolsscreen(Screen):
    ...
    def __init__(self, session):
        ...
        if dwidth == 1280:
            ...
            self.skin = '<screen position="20,center" flags="wfNoBorder" title="Pure Prestige"  size="' + str(size_w) + ',' + str(size_h) + '" > \t\t\t<eLabel position="0,0" zPosition="0" size="' + str(size_w) + ',' + str(size_h) + '" backgroundColor="' + self.color + '" /><widget name="frame" position="20,20" size="145,145" pixmap="~/pic_frame.png" zPosition="1" alphatest="on" />' + skincontent + '</screen>'
    ...
    def newPage(self):
        ...
        self['label0'].setText('Image Backup')
        self['label1'].setText('Settings Backup')
        self['label2'].setText('Settings Loader')
        self['label3'].setText('Satellites - Services')
        self['label4'].setText('Screen Grabber')
        self['label5'].setText('Swap Manager')
        self['label6'].setText('Script Manager')
        self['label7'].setText('Langauge Manager')
    ...
    def KeyOk(self):
        ...
        if self.index == 0:
            self.session.open(PPFlashBackup.plugin.TSDreamboxtypesScreen)
        if self.index == 1:
            self.session.open(SetBackupRestore.PurePrestigeBackupSettingsScrean)
        if self.index == 2:
            self.session.open(setloader.PurePrestigesettingsserver)
        if self.index == 7:
            self.session.open(PurePrestigelangsScreen)
        if self.index == 4:
        ...
        if self.index == 3:
            ...
            if HD_Res == True:
                self.session.open(PPrestigesatEditor.satedithd.PrestigePanelSatellitesEditor)
            else:
                self.session.open(PPrestigesatEditor.sateditsd.PrestigePanelSatellitesEditor)
        if self.index == 6:
            try:
