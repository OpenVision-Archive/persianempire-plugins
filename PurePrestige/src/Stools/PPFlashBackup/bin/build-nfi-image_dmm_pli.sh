...
BACKUPIMAGE=$SWAPDIR/$IMAGEINFO-$GP3$BOXTYPE-$DATE-SSL-$VSND.nfi
...
if [ -f $SSLDIR/$SND ] ; then
  ...
  echo "...Download" $SND
  wget -q http://sources.dreamboxupdate.com/download/7020/secondstage-$BOXTYPE-$VSND -O $SSLDIR/$SND > /dev/null
###########Überprüfe download####################
	if [ -f $SSLDIR/$SND ] ; then
		cp -r $SSLDIR/$SND /tmp/secondstage.bin 2> /dev/null
	else
		...
		echo "# maybe your Box or sources.dreamboxupdate.com is offline    #"
		echo "##############################################################"
		echo ""
        exit 0
	fi
