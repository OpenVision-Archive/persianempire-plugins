...
class BeautifulSOAP(BeautifulStoneSoup):
    ...
    def popTag(self):
        if len(self.tagStack) > 1:
            ...
            parent._getAttrMap()
            if isinstance(tag, Tag) and len(tag.contents) == 1 and isinstance(tag.contents[0], NavigableString) and tag.name not in parent.attrMap:
