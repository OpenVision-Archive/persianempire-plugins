...
config.plugins.navibar.hits = ConfigSelection(default='0', choices=[('0', _('AZ')), ('1', _('Hits')), ('2', _('Own'))])
config.plugins.navibar.placesize = ConfigSelection(default='450', choices=[('0', _('0')),
 ('50', _('50')),
 ('100', _('100')),
 ('150', _('150')),
 ('200', _('200')),
 ...
 ('550', _('550')),
 ('575', _('575'))])
config.plugins.navibar.fontsize = ConfigSelection(default='24', choices=[('22', _('22')),
 ('24', _('24')),
 ('26', _('26')),
 ('28', _('28'))])
config.plugins.navibar.transparent = ConfigYesNo(default=False)
config.plugins.navibar.blue = ConfigYesNo(default=False)
...
class navibar_config1(Screen, ConfigListScreen, HelpableScreen):
    ...
    def __init__(self, session, plugin_path):
        ...
        self.setTitle(_('NaviBarPE Setup ') + version)
        ...
        self.list.append(getConfigListEntry('Sort By', config.plugins.navibar.hits))
        self.list.append(getConfigListEntry('Blue Button', config.plugins.navibar.blue))
        self.list.append(getConfigListEntry('Font Size', config.plugins.navibar.fontsize))
        self.list.append(getConfigListEntry('Position', config.plugins.navibar.placesize))
        self.list.append(getConfigListEntry('Transparent', config.plugins.navibar.transparent))
        self.list.append(getConfigListEntry('Style', config.plugins.navibar.which))
        self.list.append(getConfigListEntry('Auto Close', config.plugins.navibar.close1))
        self.list.append(getConfigListEntry('Icon Path', config.plugins.navibar.dlpath))
        ...
        self['EPGSelectActions'] = HelpableActionMap(self, 'EPGSelectActions', {'input_date_time': (self.switchtpw, _('Password On/Off')),
         ...
         'prevBouquet': (self.cdown, _('Previous List'))}, -1)
        self['InfobarActions'] = HelpableActionMap(self, 'InfobarActions', {'showMovies': (self.change_section, _('Change Section'))}, -1)
        self.readconfig()
        self.readsections()
    ...
    def readconfig(self):
        ...
        for line in config_read.readlines():
            ...
            if ok:
                ...
                self.config_list_select.append((name,
                 hits,
                 hide,
                 sort,
                 pw,
    ...
    def openDirSelectScreen(self):
        self.session.openWithCallback(self.dirSelectDlgClosed, dirSelectDlg1, '/media/dummy/', False)
    ...
    def __init__(self, session, plugin_path, pluginlist, section, sectionlist):
        ...
        for each in self.pluginlist_old:
            ...
            if each[6] == self.section:
                self.dump_list.append((plugin_start,
                 name,
                 hits,
                 hide,
                 sort,
        ...
        self['actions'] = ActionMap(['OkCancelActions',
         ...
         'up': self.up,
         'down': self.down}, -1)
        self['EPGSelectActions'] = HelpableActionMap(self, 'EPGSelectActions', {'nextBouquet': self.cup,
         'prevBouquet': self.cdown}, -1)
        self.startpoint = 1
        self.select_list = 0
